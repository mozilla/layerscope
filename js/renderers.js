...
LayerScope.TreeRenderer = {
  input: function TR_input(frame) {
    // There is no layers info attached. Return directly.
    if (!frame.layers || !frame.layers.layer){
      return;
    }

    let tree = [this._createLayerNode(layer) for (layer of frame.layers.layer)];
    let layers = this._buildLayerTree(tree);
    // DFS print
