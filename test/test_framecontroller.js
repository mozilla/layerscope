...
QUnit.test("test_frameController", function(assert) {
  ...
  assert.equal(slider.slider("option", "max"), 100, "We expect min slider value to be 100");
  assert.equal(info.text(), "Frame 50/100", "We expect frame information to be 'Frame 50/100'");

  // Change current position.
  controller.update(10);
  assert.equal(slider.slider("option", "value"), 10, "We expect frame position to be 10");
  assert.equal(slider.slider("option", "max"), 100, "We expect min slider value to be 100");
  assert.equal(info.text(), "Frame 10/100", "We expect frame information to be 'Frame 10/100'");
